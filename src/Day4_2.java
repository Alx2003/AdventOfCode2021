...
public class Day4_2 {
    public static void main(String[] args) {
        ...
        // iterate through the number draws and mark each grid until the last winner is found
        breakpoint:
                ...
                for (int rowI = 0; rowI < gridSize; rowI++) {
                    for (int colI = 0; colI < gridSize; colI++) {
                        ...
                        if (draw.equals(grids[gridI][rowI][colI])) {
                            ...
                            // determine if a winner has been found and add it to list of winning grids
                            for (int check = 0; check < gridSize; check++) {
                                ...
                                if (rowMarked || colMarked) {
                                    if (!winningGrids.contains(gridI))
                                        winningGrids.add(gridI);
                                    if (winningGrids.size() == grids.length) {
        ...
        // determine score of last winning grid using given formula
        for (int i = 0; i < gridSize; i++) {
            for (int j = 0; j < gridSize; j++) {
                if (!markedGrids[winningGrids.get(winningGrids.size() - 1)][i][j]) {
                    score += Integer.parseInt(grids[winningGrids.get(winningGrids.size() - 1)][i][j]);
                }
            }
        }
        score *= lastDraw;
        System.out.println("The final score of the last winning board is: " + score + ".");
    }
}
